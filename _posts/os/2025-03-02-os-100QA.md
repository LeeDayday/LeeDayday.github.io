...
### 49. Operating System과 Kernel의 차이
...
### 57. Dispatch Latency란?
- Dispatch Latency(디스패치 지연 시간)는 System이 어떤 Process를 실행하라는 요청을 받은 후, 실제로 그 process가 실행을 시작할 때까지 걸리는 시간을 의미
- application의 우선순위를 고려하여 설계된 스케줄러를 사용할 경우, Dispatch Latency가 제한된 application도 개발이 가능
	- Real-time System(실시간 시스템)의 경우, 정해진 시간 안에 반드시 반응하는 것(predictable)이 매우 중요
		- Dispatch Latency를 제한함으로써 예측 가능한 real-time 조건을 만족하기도 함
- 즉, `스케줄링 결정 -> Switching Context -> 실행`까지의 시간

### 58. CPU Scheduling의 목표는?
- 최대 CPU 활용률 (Max CPU utilization)
	- CPU를 가능한 한 바쁘게 유지하여 자원을 최대한 활용하는 것
- 공정한 CPU 할당
	- 모든 process가 공정하게 CPU 자원을 얻도록 분배
- 최대 처리량 (Max throughput)
	- 단위 시간당 최대한 많은 process가 실행을 완료하도록 함
- 최소 반환 시간 (Min turnaround time)
	- 한 process의 실행 ~ 종료 까지 걸리는 총 시간을 최소화
- 최소 대기 시간 (Min waiting time)
	- process가 Ready Queue 에서 기다리는 시간을 최소화
- 최소 응답 시간 (Min Response time)
	- process가 처음 응답을 생성하는 데 걸리는 시간을 최소화

### 59. Critical Section이란?

- 여러 process가 동일한 코드 구간에 동시에 접근할 때, 해당 코드 구간을 Critical Section이라고 함
- Critical Section 안에는 공유 변수/자원이 포함되어 있어 데이터의 **일관성(consistency)**를 유지하기 위해 **동기화(synchronization)**가 필수
- 코드들은 **원자적(atomic)**으로 실행되어야 함
	- 실행 도중 다른 process나 thread가 끼어들 수 없음
	- 분할되지 않음

### 60. Synchronization techniques
- Mutexes (뮤텍스, 상호 배제)
	- 하나의 thread만 Critical Section에 들어갈 수 있도록 잠금을 거는 방식
- Condition Variables (조건 변수)
	- 특정 조건이 만족될 때까지 thread의 실행을 일시 정지시키는 방식
- Semaphores (세마포어)
	- resource 개수를 제어해 여러 thread의 접근을 제한하는 방식
- File locks (파일 잠금)
	- 여러 process가 동시에 파일을 수정하지 못하도록 잠금 설정
